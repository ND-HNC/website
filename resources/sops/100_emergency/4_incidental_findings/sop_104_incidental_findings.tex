...
\begin{itemize}
    ...
    \item must not cause harm to participants by \textit{commission} (e.g. false alarm leading to undue stress, unnecessary medical expenses) or \textit{omission} (e.g. withheld information leading to medical event),
    \item are required to protect participant anonymity,
    \item must adhere to IRB regulations of data usage and governance, and
    \item ought to consult with qualified individuals in case the incidental finding is medically significant.
\end{itemize}

To help research personnel navigate these obligations, we provide an escalation protocol to be followed whenever incidental findings are detected. A description of these procedures should be included in any IRB application involving use of the ND-HNC MRI scanner.
...
All MRI data collected at the ND-HNC should be reviewed by their respective research team. This typically happens during acquisition but may also occur at a later stage. In cases of incidental fingings:
...
\begin{itemize}
    \item Participant DICOM files are copied to [\textbf{TBD}], an internal, short-term data storage location.
    \item The clinician reviews the DICOM files either on-site or remotely via the University's secure network.
    \item The clinician sends their review and recommendations by email to nd-hnc@nd.edu and the director of the ND-HNC, Aron Barbey (abarbey@nd.edu).
\end{itemize}
